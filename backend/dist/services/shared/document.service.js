...
const document_analysis_service_1 = require("./document-analysis.service");
class DocumentService {
     ...
     */
    async processDocuments(provider, files, meta) {
        ...
        for (const file of files) {
            ...
            try {
                ...
                if (file.mimetype === 'application/pdf') {
                    docType = 'PDF';
                    transcription = await document_analysis_service_1.documentAnalysisService.transcribePDF(file.buffer);
                }
                else if (file.mimetype.includes('spreadsheet') || file.mimetype.includes('excel') || file.originalname.endsWith('.xlsx')) {
                    docType = 'Excel';
                    transcription = await document_analysis_service_1.documentAnalysisService.transcribeExcel(file.buffer);
                }
                else if (file.originalname.endsWith('.docx') || file.originalname.endsWith('.doc')) {
                    docType = 'DOC';
                    transcription = await document_analysis_service_1.documentAnalysisService.transcribeDoc(file.buffer);
                }
                else if (file.mimetype.startsWith('image/')) {
                    docType = 'Imagen';
                    transcription = await document_analysis_service_1.documentAnalysisService.describeImageWithGemini(file.buffer, file.mimetype);
                }
                else {
                    docType = 'Texto';
