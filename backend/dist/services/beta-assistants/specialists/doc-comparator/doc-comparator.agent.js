...
exports.docComparatorAgent = exports.DocComparatorAgent = void 0;
const messages_1 = require("@langchain/core/messages");
const google_genai_1 = require("@langchain/google-genai");
const xlsx = __importStar(require("xlsx"));
const prisma_service_1 = require("../../../shared/prisma.service");
const document_analysis_service_1 = require("../../../shared/document-analysis.service");
/**
 * ============================================================
...
class DocComparatorAgent {
     ...
     */
    extractExcelData(buffer, originalname) {
        try {
     ...
     */
    buildPdfInlinePart(buffer, mimeType) {
        return {
            type: 'media',
            mimeType: mimeType,
    ...
    async run(userMessage, files, sessionId) {
        ...
        try {
            ...
            for (const docx of docxFiles) {
                ...
                try {
                    const text = await document_analysis_service_1.documentAnalysisService.transcribeDoc(docx.buffer);
                    newDocContext += text + '\n\n';
                }
                catch (e) {
                ...
                db.prueba_chatsbeta.findFirst({ where: { session_id: sessionId } })
                    ...
                    if (existing) {
                        ...
                        return db.prueba_chatsbeta.create({
                            data: { session_id: sessionId, systemprompt_doc: docToSave, titulo: sessionId, beta_id: 'doc_comparator' }
                        });
                    }
                })
                    .then(() => console.log(`[DocComparator] 💾 Accumulated doc context saved to DB for session "${sessionId}"`))
                    .catch((e) => console.error(`[DocComparator] ❌ Error saving to DB: ${e.message}`));
            }
            // ============================================================
...
exports.DocComparatorAgent = DocComparatorAgent;
exports.docComparatorAgent = new DocComparatorAgent();
