...
async function showExecutions() {
    ...
    try {
        ...
        for (const exec of executions) {
            ...
            if (exec.provider === 'assistant')
                sourceTable = 'Asistentes de IA';
            if (exec.provider === 'pymes-assistant')
                sourceTable = 'Assistant Pymes';
            if (exec.provider === 'beta-assistant')
                sourceTable = 'Beta Assistants';
            ...
            if (inputSnippet && inputSnippet.length > 80)
                inputSnippet = inputSnippet.substring(0, 80) + '...';
            console.log(`-> Input (Truncado): ${inputSnippet}`);
            let outputSnippet = JSON.stringify(exec.output);
            if (outputSnippet && outputSnippet.length > 80)
                outputSnippet = outputSnippet.substring(0, 80) + '...';
