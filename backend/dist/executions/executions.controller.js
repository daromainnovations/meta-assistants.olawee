    ...
    try {
        ...
        if (search) {
            ...
            if (where.environment) {
                params.push(where.environment);
                queryStr += ` AND environment = $${params.length}`;
            }
            if (where.status) {
        ...
        const statsParams = [];
        if (where.category) {
            statsParams.push(where.category);
            statsQuery += ` AND category = $${statsParams.length}`;
        }
        if (where.environment) {
            statsParams.push(where.environment);
            statsQuery += ` AND environment = $${statsParams.length}`;
        }
        ...
        let chartQuery = `
            ...
            WHERE created_at > $1
        `;
        const chartParams = [last24h];
        if (where.category) {
            chartParams.push(where.category);
            chartQuery += ` AND category = $${chartParams.length}`;
        }
        if (where.environment) {
            chartParams.push(where.environment);
            chartQuery += ` AND environment = $${chartParams.length}`;
        }
        chartQuery += ` GROUP BY 1 ORDER BY 1 ASC`;
        console.log(`[API] Chart query: ${chartQuery} [${chartParams}]`);
        const chartData = await db.$queryRawUnsafe(chartQuery, ...chartParams);
        ...
        let weeklyQuery = `
            ...
            WHERE created_at > $1
        `;
        const weeklyParams = [last7d];
        if (where.category) {
            weeklyParams.push(where.category);
            weeklyQuery += ` AND category = $${weeklyParams.length}`;
        }
        if (where.environment) {
            weeklyParams.push(where.environment);
            weeklyQuery += ` AND environment = $${weeklyParams.length}`;
        }
        weeklyQuery += ` GROUP BY 1 ORDER BY 1 ASC`;
        const weeklyData = await db.$queryRawUnsafe(weeklyQuery, ...weeklyParams);
        res.json({
            executions,
            stats: {
...
async function retryExecution(req, res) {
    try {
        ...
        const result = await webhook_service_1.webhookService.handleIncomingRequest(previous.provider === 'assistant' ? 'assistant' : (previous.category === 'meta' ? 'meta-assistant' : previous.provider), previous.input, [] // Files se pierden en el retry si se guardaron en memoria (limitación aceptada)
        );
